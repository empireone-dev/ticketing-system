...
import { create_ticket_service, export_generate_ticket_service, get_ticket_by_id_service, get_ticket_service, update_ticket_status_service } from "@/app/services/ticket-service";
import { get_activity_by_id_service } from "@/app/services/activity-service";
import { add_notes_service, get_notes_by_id_service } from "@/app/services/note-service";
...
export function export_generate_ticket_thunk(data) {
    return async function (dispatch, getState) {
        const result = await export_generate_ticket_service(data);
        // dispatch(ticketSlice.actions.setActivity(result.data.result));
        return result;
    };
}
...
export function add_notes_thunk(data) {
    return async function (dispatch, getState) {
        const result = await add_notes_service(data);
        // dispatch(ticketSlice.actions.setNotes(result.data.result));
        return result;
    };
}
